...
public function store(Request $request)
{
    ...
    $request->validate([
        ...
        'project_img' => 'required|image|mimes:jpeg,png,jpg,gif|max:2048',
    ]);

    // Image upload
    $project_img = $request->file('project_img');
    $imgExtension = Str::lower($project_img->getClientOriginalExtension());
    $imgName = time() . Str::random(25) . '.' . $imgExtension;

    try {
        $stored = Storage::putFileAs('project_images', $project_img, $imgName);

        if (!$stored) {
            return back()->withErrors(['project_img' => 'Failed to upload image'])->withInput();
        }
        ...
        return to_route('dashboard')->with('message', 'Project created successfully!');
